# ============================================================================
# app_v3.py — E-Nose 測試伺服器 v3（包含 Agent AI 整合）
# ============================================================================
...
from google import genai  # 🌟 匯入 Gemini 套件
...
# 🌟 Gemini API 設置 (請在此換成你們的 API Key)
...
def on_device_message(msg, client):
    ...
    if msgtype == 'RESULT' and len(parts) >= 6:
        ...
        ai_advice_text = "系統忙碌中，暫時無法取得 AI 建議。"
        try:
            ...
            print(f"[AI] 呼叫失敗: {e}")
            ai_advice_text = f"AI 建議生成失敗 ({e})"

        # 🌟 將 ai_advice 塞進 result 裡，打包給前端
        meta = pending_tasks.pop(tid, {'created_at': time.time()})
        completed_tasks[tid] = {
            'status':     'done',
            'type':       'detailed',
            ...
            'result': {
                ...
                'curr_ppm':       curr_ppm,
                'meal_state':     meal_state,
                'ai_advice':      ai_advice_text  # 讓 UI 顯示
            }
        }
...
if __name__ == '__main__':
    ...
    print("="*50)
    print("  E-Nose v3 Server (含 Agent AI)")
