...
float ADS1115_ReadVoltage_Diff_A0_A1(void);
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* 2. 讀取 ADS1115 電壓 (A0, A1 差動) */
    float ads_volt = ADS1115_ReadVoltage_Diff_A0_A1();

    /* 3. 輸出到 UART (電腦端觀察) */
    printf("Tgs1820: %.4f V | A1(MQ137): %.2fV | A2(MQ3): %.2fV | A3(MQ136): %.2fV\r\n",
           ads_volt, gVoltage[0], gVoltage[2], gVoltage[1]);

    /* 4. 輸出到 OLED (LCD端觀看) */
    sprintf(oled_buf, "Tgs1820: %.3f V", (double)ads_volt);
    OLED_DrawString(0, 0, oled_buf);
    
    sprintf(oled_buf, "MQ137 : %.3f V", (double)gVoltage[0]);
    OLED_DrawString(2, 0, oled_buf);
    
    sprintf(oled_buf, "MQ136 : %.3f V", (double)gVoltage[1]);
    OLED_DrawString(4, 0, oled_buf);

    sprintf(oled_buf, "MQ3 : %.3f V", (double)gVoltage[2]);
...
/*********************************************************************************************************//**
  * @brief  ADS1115 讀取 A0, A1 差動電壓
  ***********************************************************************************************************/
float ADS1115_ReadVoltage_Diff_A0_A1(void) {
  u8 config_msb = 0x83; /* 開始單次轉換, AIN0-AIN1 差動, +/- 4.096V, 單次模式 */
